  ...
  var createLogicGateFactory = function(op, out, draw, gateClass) {
    return function(device) {
        ...
        device.createUI = function() {
            ...
            g.attr['class'] += gateClass; // Add gateClass
            draw(g,
                (size.width - unit) / 2,
                (size.height - unit) / 2,
                unit, unit);
...
// Modify the logical gate definitions to include gate classes
var BUF = createLogicGateFactory(null, BUF, drawBUF, 'simcir-basicset-buf');
var AND = createLogicGateFactory(AND, BUF, drawAND, 'simcir-basicset-and');
var OR = createLogicGateFactory(OR, BUF, drawOR, 'simcir-basicset-or');
var NOT = createLogicGateFactory(null, NOT, drawNOT, 'simcir-basicset-not');
var NAND = createLogicGateFactory(AND, NOT, drawNAND, 'simcir-basicset-nand');
var NOR = createLogicGateFactory(OR, NOT, drawNOR, 'simcir-basicset-nor');
var EOR = createLogicGateFactory(EOR, BUF, drawEOR, 'simcir-basicset-xor'); // Assuming ENOR function exists
var ENOR = createLogicGateFactory(ENOR, BUF, drawENOR, 'simcir-basicset-xnor'); // Assuming ENOR function exists
